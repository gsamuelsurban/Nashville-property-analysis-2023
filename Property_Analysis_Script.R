...
#regular expressions. Subsetting institutions by institution type, using words
#and phrases that one might expect to see in their names.
library(stringr)

#faith-based organizations
regex_faith <- "(?i)\\b(church(?!\\s+st)|(baptist?!\\s +hospital)|catholic|synagogue|mosque|congregation|beth|beit|god|lord|shalom)\\b"

faith_based <- Q1[str_detect(Q1$ownername1full, regex_faith) & Q1$owner1corpind |str_detect(Q1$ownername2full, regex_faith) & Q1$owner2corpind,]

#health organizations - excluding healthcare, for now
regex_health <- "(?i)\\b(hospital|medical center|clinic|medical care|hca health services)\\b"

health <- Q1[str_detect(Q1$ownername1full, regex_health) & Q1$owner1corpind |str_detect(Q1$ownername2full, regex_health) & Q1$owner2corpind,]

#education
regex_education <- "(?i)(\\buniversity\\b(?!\\s+\\w+)|college)"
education <- Q1[str_detect(Q1$ownername1full, regex_education) & Q1$owner1corpind |str_detect(Q1$ownername2full, regex_education) & Q1$owner2corpind,]

#sports
regex_sports <- "(?i)(sports|titans|predators|sounds|soccer club)"
sports <- Q1[str_detect(Q1$ownername1full, regex_sports) & Q1$owner1corpind |str_detect(Q1$ownername2full, regex_sports) & Q1$owner2corpind,]
